...
void StartFadeOut(signed char dir) {
        ...
        for (y = 0; y < FADE_HEIGHT; ++y) {
          for (x = 0; x < FADE_WIDTH; ++x) {
            gFade.ani_no[COMPOUND_2D_ACCESS(y, x)] /*[y][x]*/ = 0;
            gFade.flag[COMPOUND_2D_ACCESS(y, x)] /*[y][x]*/ = FALSE;
          }
        }
}
...
void StartFadeIn(signed char dir) {
        ...
        for (y = 0; y < FADE_HEIGHT; ++y) {
          for (x = 0; x < FADE_WIDTH; ++x) {
            gFade.ani_no[COMPOUND_2D_ACCESS(y, x)] /*[y][x]*/ = 15;
            gFade.flag[COMPOUND_2D_ACCESS(y, x)] /*[y][x]*/ = FALSE;
          }
        }

        x = x;  // This probably doesn't match the original source code, but it
...
void ProcFade(void)
{
	...
	switch (gFade.mode)
	{
          case 2:
            switch (gFade.dir) {
                ...
                for (y = 0; y < FADE_HEIGHT; ++y)
                  for (x = 0; x < FADE_WIDTH; ++x)
                    if ((FADE_WIDTH - 1) - gFade.count == x)
                      gFade.flag[COMPOUND_2D_ACCESS(y, x)] /*[y][x]*/
                          = TRUE;
              ...
              case 4:
                ...
                for (y = (FADE_HEIGHT / 2); y < FADE_HEIGHT; ++y)
                  for (x = (FADE_WIDTH / 2); x < FADE_WIDTH; ++x)
                    if (gFade.count ==
                        ((FADE_WIDTH - 1) - x) + ((FADE_HEIGHT - 1) - y))
                      gFade.flag[COMPOUND_2D_ACCESS(y, x)] /*[y][x]*/
                          = TRUE;
            ...
            for (y = 0; y < FADE_HEIGHT; ++y)
              for (x = 0; x < FADE_WIDTH; ++x)
                if (gFade.ani_no[COMPOUND_2D_ACCESS(y, x)] /*[y][x]*/
                        < 15 &&
                    gFade.flag[COMPOUND_2D_ACCESS(y, x)] /*[y][x]*/)
                  ++gFade.ani_no[COMPOUND_2D_ACCESS(y, x)] /*[y][x]*/;

            if (++gFade.count >
                ((FADE_WIDTH > FADE_HEIGHT) ? FADE_WIDTH : FADE_HEIGHT) + 16) {
              gFade.bMask = TRUE;
              gFade.mode = 0;
            }
...
void PutFade(void)
{
	...
	for (y = 0; y < FADE_HEIGHT; ++y)
	{
                for (x = 0; x < FADE_WIDTH; ++x) {
                  ...
                  PutBitmap3(&grcGame, PixelToScreenCoord(x * 16),
                             PixelToScreenCoord(y * 16), &rect,
                             SURFACE_ID_FADE);
                }
        }
}
